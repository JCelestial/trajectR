...
Inspired by the Density calculations in Theoretical Chemistry and Statistical Mechanics, **trajectR** will make density statistics much easier to calculate visualize!
...
<img src="https://github.com/JCelestial/trajectR/blob/master/density1.jpg" width="275" height="300">
...
This is the problem **trajectR** aims to solve by providing functions that will allow one to organize data in such a way without requiring SQL libraries to manipulate the data frames using keys and joins. **trajectR** also contains other parameters that makes the resulting data more customizable.

------------------------------------------------------------------------------------------------------------------------------------
### Features
Now that we know what **trajectR** is used for, let's take a look at the bells and whistles!
...
In addition, **trajectR** contains functions that also allow one to compare multiple tables and join them based on conditions, i.e. creating a new table with entries from either data frames that are compared line per line. Quite similar to how SQL makes use of joins and unions.

*Compatibility with packages such as DplyR and magrittR pipelines!*

**trajectR** functions can be included as part of data munging done with DplyR.
